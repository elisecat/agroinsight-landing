    ...
    <nav class="bg-white">
                ...
                <div class="hidden sm:block sm:ml-6">
                    <div class="flex space-x-4">
                        <router-link v-for="route in routes" :key="route.path" :to="route.path"
                            class="text-green-700 hover:bg-green-500 hover:text-white px-3 py-2 rounded-md text-sm font-medium flex items-center space-x-2">
        ...
        <div v-if="isOpen" class="sm:hidden">
            <div class="px-2 pt-2 pb-3 space-y-1">
                <router-link v-for="route in routes" :key="route.path" :to="route.path"
                    class="text-green-700 hover:bg-green-500 hover:text-white block px-3 py-2 rounded-md text-base font-medium items-center space-x-2">
                    <i :class="route.icon" class="text-lg"></i>
                    <span>{{ route.name }}</span>
                </router-link>
            </div>
